(async function(){
  ...
  function renderNotions(items){
    list.innerHTML = items.map(n => `<article class="card item" id="notion-${n.id}">
      ...
      <button class="btn" data-id="notion-${n.id}">Ouvrir la fiche</button>
    </article>`).join('');

    list.querySelectorAll('[data-id]').forEach((b)=>b.onclick = ()=>{
      const notionKey = b.dataset.id.replace('notion-','').toLowerCase();
      const n = items.find((x)=>String(x.id).toLowerCase()===notionKey);
      ...
      ${(n.schemas_theme||[]).map((id)=>{ const m=mediaById(id); return m?`<figure class='schema-figure'><img loading='lazy' class='zoomable-img' src='${m.src}' alt='${m.alt}'><figcaption>${m.title}</figcaption><button class='btn secondary zoom-btn' data-zoom-src='${m.src}' data-zoom-alt='${m.alt}'>🔍 Agrandir</button></figure>`:''; }).join('')}
  ...
  protList.innerHTML = protocoles.map((p)=>{ const m=mediaById(p.schema_visuel); return `<article class='card item'><h4>${p.titre}</h4><ol>${(p.etapes||[]).map((e)=>`<li>${e}</li>`).join('')}</ol><p><strong>Vigilance:</strong> ${(p.vigilance||[]).join(' ; ')}</p>${m?`<figure class='schema-figure'><img loading='lazy' class='zoomable-img' src='${m.src}' alt='${m.alt}'><figcaption>Support visuel protocole</figcaption><button class='btn secondary zoom-btn' data-zoom-src='${m.src}' data-zoom-alt='${m.alt}'>🔍 Agrandir</button></figure>`:''}</article>`; }).join('');
  ...
  if (location.hash.startsWith('#notion-')) {
    const targetId = location.hash.replace('#','').toLowerCase();
    const btn = Array.from(document.querySelectorAll('[data-id]')).find((el)=>el.dataset.id.toLowerCase()===targetId);
    if (btn) btn.click();
  }
})();
